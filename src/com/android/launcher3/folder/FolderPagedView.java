...
import com.android.launcher3.big.add.HxyAddBubbleTextView;
...
public class FolderPagedView extends PagedView<PageIndicatorDots> implements ClipPathView {
     ...
     */
    public void bindItems(List<WorkspaceItemInfo> items, boolean addFlag) {
        if (mViewsBound) {
            unbindItems();
        }
        List<View> list = new ArrayList<>();
        for (WorkspaceItemInfo item : items) {
            list.add(createNewView(item));
        }
        WorkspaceItemInfo addInfo = new WorkspaceItemInfo();
        addInfo.title = getContext().getResources().getString(R.string.str_add_to);
    ...
    public View createAndAddViewForRank(WorkspaceItemInfo item, int rank) {
        ...
        if (rank == views.size()) {
            if (views.contains(mAdd)) {
                int index = views.indexOf(mAdd);
                views.add(index, icon);
            } else {
                views.add(rank, icon);
            }
        } else {
        ...
        arrangeChildren(views, true);
        View view = this.mAdd;
        if (view != null) {
            ((ItemInfo) view.getTag()).rank = rank + 1;
        }
        return icon;
    }
    ...
    public View createAddView(WorkspaceItemInfo item) {
        if (item == null) {
            return null;
        }
        HxyAddBubbleTextView textView = this.mViewCache.getView(R.layout.add_folder_application, getContext(), null);
        textView.setOnClickListener(textView);
        textView.setText(item.title);
        textView.setIcon(new FastBitmapDrawable(((BitmapDrawable) getResources().getDrawable(R.drawable.add_folder_icon)).getBitmap()));
        textView.setOnFocusChangeListener(this.mFocusIndicatorHelper);
        textView.setTag(item);
        textView.setFolder(this.mFolder);
        CellLayoutLayoutParams lp = (CellLayoutLayoutParams) textView.getLayoutParams();
        if (lp == null) {
    ...
    public void removeItem(View v, boolean removeAddView) {
        for (int i = getChildCount() - 1; i >= 0; i --) {
            getPageAt(i).removeView(v);
            if (removeAddView) {
                getPageAt(i).removeView(mAdd);
            }
        }
    }
    ...
    public void arrangeChildren(List<View> list, boolean addFlag) {
        ...
        mFolder.mFolderName.setGravity(getPageCount() > 1 && !LauncherApplication.getContext().getResources().getBoolean(R.bool.config_show_full_folder_style) ?
