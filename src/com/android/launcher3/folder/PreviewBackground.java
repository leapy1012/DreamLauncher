...
public class PreviewBackground extends CellLayout.DelegatedCellDrawing {
    ...
    public void drawUnderItem(Canvas canvas) {
        drawThemeBackground(canvas, this.mContext);
        if (!isClipping) {
    ...
    private void resetPreviewPaint() {
    ...
    private void drawPreviewBackground(Canvas canvas, Context context) {
        resetPreviewPaint();
        this.mRectF.left = (float) this.basePreviewOffsetX;
        this.mRectF.top = (float) this.basePreviewOffsetY;
        RectF rectF = this.mRectF;
        rectF.right = rectF.left + ((float) this.mPreviewWidth);
        RectF rectF2 = this.mRectF;
        rectF2.bottom = rectF2.top + ((float) this.mPreviewHeight);
        float f = this.mScale;
        canvas.scale(f, f, this.mRectF.left + (this.mRectF.width() / 2.0f), this.mRectF.top + (this.mRectF.height() / 2.0f));
        drawPreviewBackground(canvas, context, this.mRectF);
    }
