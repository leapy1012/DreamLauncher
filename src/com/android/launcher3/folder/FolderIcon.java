...
public class FolderIcon extends FrameLayout implements FolderListener, IconLabelDotView,
    ...
    public @Thunk ActivityContext mActivity;
    ...
    private NumberDotRenderer mDotRenderer;
    @ViewDebug.ExportedProperty(category = "launcher", deepExport = true)
    private NumberDotRenderer.DrawParams mDotParams;
    ...
    private boolean willAcceptItem(ItemInfo item) {
        ...
        return ((itemType == LauncherSettings.Favorites.ITEM_TYPE_APPLICATION ||
                ...
                item != mInfo && !mFolder.isOpen() && !isYftWidget(item));
    }
    ...
    public void onDrop(DragObject d, boolean itemReturnedOnFailedDrop) {
        ...
        if (d.dragInfo instanceof WorkspaceItemFactory) {
            ...
            mFolder.notifyDrop();
            for (WorkspaceItemInfo child : ((FolderInfo) d.dragInfo).contents) {
                onDrop(child, d, null, 1.0f,
                        itemReturnedOnFailedDrop ? child.rank : mInfo.contents.size(),
    ...
    public void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (isLargeFolder) {
            ...
            int cellHeightPx = this.mFolderName.getIconSize() + this.mActivity.getDeviceProfile().iconDrawablePaddingPx * 2 + ((int) Math.ceil((double) (fm.bottom - fm.top)));
            int left = getPaddingLeft();
            int right = getPaddingRight();
            int top = measurePaddingTop(View.MeasureSpec.getSize(heightMeasureSpec), cellHeightPx);
            int bottom = getPaddingBottom();
            android.util.Log.d("FolderIcon", "onMeasure: left=" + left + ", top=" + top + ", right=" + right + ", bottom=" + bottom);
    ...
    public int measurePaddingTop(int height, int cellHeightPx) {
        ...
        if (isInHotseat()) {
            ...
            if (grid.isMultiWindowMode) {
                paddingTop = (0 - ((height - cellHeightPx) / 4)) + 2;
            } else {
    ...
    public void drawPreviewBackground(Canvas canvas) {
        ...
        if (!this.mBackground.drawingDelegated()) {
            this.mBackground.drawThemeBackground(canvas, getContext());
        }
    }
    ...
    public void dispatchDrawEnd(Canvas canvas) {
        if (!this.mCurrentPreviewItems.isEmpty() || this.mAnimating) {
            ...
            drawPreviewItems(canvas, this.mPreviewItemManager);
    ...
    public void onFolderAnimStartClose(int currentPage) {
        mPreviewItemManager.onFolderAnimStartClose(currentPage);
    }
    ...
    public Rect getIconRect() {
        int iconSize = ((Launcher) mActivity).getDeviceProfile().folderIconSizePx;
