...
//Description:当系统带有filesgo的情况下，桌面不显示downloads
...
public class AllAppsList {
    ...
    private boolean checkAppInstalled(Context context, String pkgName){ 
        ...
        for(int i = 0; i < info.size(); i++ ){
            if(pkgName.equals(info.get(i).packageName) && ((info.get(i).applicationInfo.flags & ApplicationInfo.FLAG_SYSTEM) != 0)){
                return true;
            }
        }
        return false;
    ...
    public void add(AppInfo info, LauncherActivityInfo activityInfo, Context context) {
    ///&&}}	
    ...
    public void add(AppInfo info, LauncherActivityInfo activityInfo, boolean loadIcon, Context context) {
        ...
        ///Hxy:add by yangsong for hide downloads when have filesgo{{&&
        //Description:当系统带有filesgo的情况下，桌面不显示downloads
        if ("com.android.documentsui".equals(info.componentName.getPackageName())
        ||"com.google.android.documentsui".equals(info.componentName.getPackageName())) {
            if(checkAppInstalled(context , "com.google.android.apps.nbu.files")) {
                return;
            }
        }
        ///&&}}
        if ("com.android.inputmethod.latin".equals(info.componentName.getPackageName())) {//Launcher过滤AOSP键盘
            return;
        }
    ...
    public List<LauncherActivityInfo> addPackage(
        ...
        for (LauncherActivityInfo info : activities) {
            ...
            add(new AppInfo(context, info, user), info, context);
            ///&&}}
        }
