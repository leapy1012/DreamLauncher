...
public class LoaderTask implements Runnable {
    ...
    public void run() {
        ...
        try (LauncherModel.LoaderTransaction transaction = mApp.getModel().beginLoader(this)) {
            ...
            if (mApp.getInvariantDeviceProfile().dbFile.equals(mDbName)) {
                ...
                if (mApp.getContext().getSystemService(android.os.UserManager.class).isUserUnlocked()) {
            ...
            if (mApp.getContext().getSystemService(android.os.UserManager.class).isUserUnlocked()) {
                ...
                if (LauncherPrefs.getPrefs(mApp.getContext()).getBoolean(LauncherPrefs.WORKSPACE_MEMORY_CLEAN, true)) {
                    addMemoryClean();
                }
                if (LauncherPrefs.getPrefs(mApp.getContext()).getBoolean(LauncherPrefs.WORKSPACE_WALLPAPER_SET, true)) {
                    addWallpaperSet();
                }
            }
    ...
    private void loadWorkspaceImpl(
        ...
        if (TextUtils.isEmpty(selection) && !mApp.getModel().getModelDbController().migrateGridIfNeeded()) {
        ...
        try {
            ...
            switch (c.itemType) {
                ...
                case Favorites.ITEM_TYPE_DEEP_SHORTCUT:
                    ...
                    //hxy-bug clone app bug 20240416 by yangj
                    boolean isClone = isUserTypeProfile(mApp.getContext(), c.user,
                        UserManager.USER_TYPE_PROFILE_CLONE);
                    if(isClone && mSupportAppsList.contains(targetPkg)) {
                        //validTarget = false;
                    }
                   //hxy-bug clone app bug 20240416 by yangj
        ...
        for (UserHandle user : profiles) {
           ...
           //hxy-bug clone app bug 20240416 by yangj
            ...
            for (int i = 0; i < apps.size(); i++) {
                ...
                ///Hxy:add by yangsong for hide downloads when have filesgo{{&&
                //mBgAllAppsList.add(
                //        appInfo, app, false);
                mBgAllAppsList.add(
                        appInfo, app, false, mApp.getContext());
                ///&&}}
            }
            ...
            for (int i = 0; i < mBgDataModel.folders.size(); i++) {
                ...
                if (info.getLabelState().equals(FolderInfo.LabelState.SUGGESTED)) {
                    try {
                        ...
                        Log.e(TAG, "loadFolderNames: " + e.getMessage());
                    }
                }
    ...
    private String getDefaultName(int id) {
        ...
        final Partner partner = Partner.get(mApp.getContext().getPackageManager());
        try {
            defaultFolderName = mApp.getContext().getResources().getString(id);
            if (partner != null) {
                final Resources partnerRes = mApp.getContext().getPackageManager().getResourcesForApplication(partner.getPackageName());
                // final int resId = partnerRes.getIdentifier(name, "string", partner.getPackageName());
                defaultFolderName = partnerRes.getString(id);
            }
        } catch (Exception e) {
            Log.e(TAG, "getDefaultName: " + e.getMessage());
        }
    ...
    private void addMemoryClean() {
        Context context = mApp.getContext();
        ArrayList<Pair<ItemInfo, Object>> installQueue = new ArrayList<>();
        List<UserHandle> profiles = context.getSystemService(LauncherApps.class).getProfiles();
        for (UserHandle user : profiles) {
            if (isUserTypeProfile(context, user, UserManager.USER_TYPE_PROFILE_CLONE)) {
                continue;
            }
            ItemInstallQueue.PendingInstallShortcutInfo pendingInstallShortcutInfo
             = new ItemInstallQueue.PendingInstallShortcutInfo(BuildConfig.APPLICATION_ID, user);
            installQueue.add(pendingInstallShortcutInfo.getMemoryInfo(context));
        }
        mApp.getModel().addAndBindAddedWorkspaceItems(installQueue);
    }

    private void addWallpaperSet() {
        Context context = mApp.getContext();
        ArrayList<Pair<ItemInfo, Object>> installQueue = new ArrayList<>();
        List<UserHandle> profiles = context.getSystemService(LauncherApps.class).getProfiles();
        for (UserHandle user : profiles) {
            ...
            ItemInstallQueue.PendingInstallShortcutInfo pendingInstallShortcutInfo
             = new ItemInstallQueue.PendingInstallShortcutInfo(BuildConfig.APPLICATION_ID, user);
            installQueue.add(pendingInstallShortcutInfo.getWallpaperInfo(context));
        }
        mApp.getModel().addAndBindAddedWorkspaceItems(installQueue);
    }
    ...
    private void verifyApplications() {
        ...
        ArrayList<Pair<ItemInfo, Object>> installQueue = new ArrayList<>();
         //hxy-bug clone app bug 20240416 by yangj
        final List<UserHandle> profiles = context.getSystemService(LauncherApps.class).getProfiles();//mUserManager.getUserProfiles();
         //hxy-bug clone app bug 20240416 by yangj
        for (UserHandle user : profiles) {
            final List<LauncherActivityInfo> apps = mLauncherApps.getActivityList(null, user);
    ...
    public static boolean isUserTypeProfile (Context context, UserHandle userHandle, String type) {
        ...
        if(userInfo != null)
        //android.util.Log.d("Launcher3", "userInfo.isCloneProfile() "+userInfo.isCloneProfile()+"--type:"+type);
        switch (type) {
