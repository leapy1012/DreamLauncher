...
public class PackageUpdatedTask extends BaseModelUpdateTask {
    ...
    public void execute(@NonNull final LauncherAppState app, @NonNull final BgDataModel dataModel,
        ...
        switch (mOp) {
            case OP_ADD: {
                 //hxy-bug clone app bug 20240416 by yangj
                if (N == 1 && (isCloneAppNotShowing(context, mUser, packages[0]) || isUserTypeProfile(context, mUser, UserManager.USER_TYPE_PROFILE_CLONE))) {
                    flagOp = FlagOp.NO_OP;
                    break;
                }
                 //hxy-bug clone app bug 20240416 by yangj
                for (int i = 0; i < N; i++) {
                    ...
                    if (launcher != null) {
                        launcher.updateAppInstallationBadge(packages[i]);
                    }
                    // liu-db add end
                }
            ...
            case OP_UNAVAILABLE:
                ...
                Launcher launcher = Launcher.ACTIVITY_TRACKER.getCreatedActivity();
                boolean status = LauncherPrefs.getPrefs(context).getBoolean(LauncherPrefs.WORKSPACE_FILL_CELL, false);
                if (launcher != null && status) {
                    launcher.setAutoFull(true);
		...
		final ArrayList<Pair<ItemInfo, Object>> installQueue = new ArrayList<>();
        if (mOp == OP_REMOVE) {
                ...
                if (!launcherApps.isPackageEnabled(packages[i], mUser)) {
                    ...
                    if (android.provider.Settings.System.getInt(context.getContentResolver(), "launcher_style", 0) == 0
                     && !SessionCommitReceiver.isEnabled(context)) {
                        if (DEBUG) {
                            Log.d(TAG, "OP_UPDATE:"
                                    + " package " + packages[i] + " is not on home screen, launcher style is drawer ignore.");
                        }
                    } else {
                        ItemInstallQueue.PendingInstallShortcutInfo pendingInstallShortcutInfo  =  new ItemInstallQueue.PendingInstallShortcutInfo(packages[i], mUser);
        ...
        if (!removedPackages.isEmpty() || !removedComponents.isEmpty()) {
		  ...
		  } else if (mOp == OP_UPDATE && !installQueue.isEmpty()) {
        ...
        if (mOp == OP_ADD) {
         ...
         //hxy-bug clone app bug 20240416 by yangj
    private  String[] mSupportApps;
    private  List<String> mSupportAppsList;
         //hxy-bug clone app bug 20240416 by yangj
    //hxy-feature: add launcher style function  202312
    public void updateToWorkSpace(Context context, LauncherAppState app , AllAppsList appsList){
        ...
        = new ArrayList<ItemInstallQueue.PendingInstallShortcutInfo>();
         //hxy-bug clone app bug 20240416 by yangj
        mSupportApps = context.getResources()
        ...
        for (UserHandle user : profiles) {
         ...
         //hxy-bug clone app bug 20240416 by yangj
            boolean isClone = isUserTypeProfile(context, user,
                             UserManager.USER_TYPE_PROFILE_CLONE);
            synchronized (this) {
                for (LauncherActivityInfo info : apps) {
                    for (AppInfo appInfo : appsList.data) {
                        if(info.getComponentName().equals(appInfo.componentName)){
                            ItemInstallQueue.PendingInstallShortcutInfo mPendingInstallShortcutInfo=null;
                             if (isClone) {
                                 if(mSupportAppsList.contains(info.getComponentName().getPackageName())) {
                                     mPendingInstallShortcutInfo  =  new ItemInstallQueue.PendingInstallShortcutInfo(info.getComponentName().getPackageName(), info.getUser());
	...
	private boolean isAppOnHomeScreen(String packageName, UserHandle user, BgDataModel dataModel) {
        Optional<ItemInfo> foundItem = dataModel.workspaceItems.stream()
                .filter(itemInfo -> itemInfo.user.equals(user)
                        && itemInfo.getTargetComponent() != null
                        && itemInfo.getTargetComponent().getPackageName().equals(packageName))
                .findAny();
