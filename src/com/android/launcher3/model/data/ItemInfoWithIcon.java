...
//hxy_leifengqi update to Dynamic clock 20230303 start
import com.android.launcher3.DynamicClockIcon;
...
public abstract class ItemInfoWithIcon extends ItemInfo {
    ...
    public FastBitmapDrawable newIcon(Context context, @DrawableCreationFlags int creationFlags) {
        ...
        //hxy_leifengqi update to Dynamic clock 20230303 start
        String pkg = context.getResources().getString(R.string.dynamic_clock_pkg);
		ComponentName componentName = this.getTargetComponent();
        if(componentName != null) {
			if (componentName.getPackageName().equals(pkg)) {
                ...
                if (themedName.contains("hills")) {
                    ...
                    resId = R.drawable.theme_tunnel_alarmclock;
                } else if (themedName.contains("forest")) {
                    resId = R.drawable.theme_forest_alarmclock;
                } else if (themedName.contains("winter")) {
                    resId = R.drawable.theme_winter_alarmclock;
                } else if (themedName.contains("golden")) {
                    resId = R.drawable.theme_golden_alarmclock;
                } else if (themedName.contains("night")) {
                    resId = R.drawable.theme_night_alarmclock;
                } else if (themedName.contains("minimalist")) {
                    resId = R.drawable.theme_default_alarmclock;
                } else if (themedName.contains("glow")) {
                    resId = R.drawable.theme_glow_alarmclock;
                } else if (themedName.contains("supercar")) {
                    resId = R.drawable.theme_supercar_alarmclock;
                } else if (themedName.contains("stillness")) {
                    resId = R.drawable.theme_stillness_alarmclock;
                }
                if (themedName.contains("starry")) {
                    drawable = bitmap.newIcon(context, creationFlags);
                } else {
                    drawable = new DynamicClockIcon(context, resId);
                }
            } else {
                drawable = bitmap.newIcon(context, creationFlags);
            }
        }
        //hxy_leifengqi update to Dynamic clock 20230303 end
