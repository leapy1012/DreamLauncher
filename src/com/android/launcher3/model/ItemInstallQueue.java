...
public class ItemInstallQueue {
    ...
    public static class PendingInstallShortcutInfo extends ItemInfo {///hxy-feature: add launcher style function  202312
        ...
        public Pair<ItemInfo, Object> getMemoryInfo(Context context) {
            ...
            WorkspaceItemInfo si = new WorkspaceItemInfo();
            si.user = user;
            si.title = context.getString(R.string.app_name_memory_clean);
            si.contentDescription = context.getString(R.string.app_name_memory_clean);
            si.intent = makeLaunchIntent(memory).setPackage(BuildConfig.APPLICATION_ID);
            si.bitmap = LauncherIcons.obtain(context).createBadgedIconBitmap(Objects.requireNonNull(ResourcesCompat.getDrawable(context.getResources(), R.drawable.hxy_clear_bg, null)), null, Color.rgb(25, 212, 214));
            return Pair.create(si, null);
        }

		public Pair<ItemInfo, Object> getWallpaperInfo(Context context) {
            ComponentName memory = new ComponentName(BuildConfig.APPLICATION_ID, "com.android.launcher3.settings.WallpaperChangeActivity");
            WorkspaceItemInfo si = new WorkspaceItemInfo();
            si.user = user;
            si.title = context.getString(R.string.wallpaper_choose_widget_title);
            si.contentDescription = context.getString(R.string.wallpaper_choose_widget_title);
            si.intent = makeLaunchIntent(memory).setPackage(BuildConfig.APPLICATION_ID);
            si.bitmap = LauncherIcons.obtain(context).createBadgedIconBitmap(Objects.requireNonNull(ResourcesCompat.getDrawable(context.getResources(), R.mipmap.choosewallpapaer_bg, null)));
            return Pair.create(si, null);
        }
            ...
            switch (itemType) {
                case ITEM_TYPE_APPLICATION: {
                    ...
                    if (usePackageIcon) {
                        ...
                        lai = pickWorkspaceLauncherActivity(laiList);
