...
import com.android.launcher3.big.IconSizeSettingActivity;
...
public class DeviceProfile {
    ...
    // Notification dots
    public NumberDotRenderer mDotRendererWorkSpace;
    public NumberDotRenderer mDotRendererAllApps;
    ...
    private boolean showHotseatText;
    private int hotseatTextSizePx;
    ...
    DeviceProfile(Context context, InvariantDeviceProfile inv, Info info, WindowBounds windowBounds,
        ...
        if (isScalableGrid && inv.folderStyle != INVALID_RESOURCE_HANDLE) {
            ...
            folderContentPaddingTop = LauncherPrefs.getPrefs(context).getBoolean(LauncherPrefs.WORKSPACE_APP_NAME, true) ? res.getDimensionPixelSize(R.dimen.folder_top_padding_default) : 0;
        }
        ...
        cellStyle.recycle();
        // Oppo General display-option: oplusIconDrawablePaddingDp (integers.xml).
        if (mUseOppoWorkspaceMetrics) {
            iconDrawablePaddingOriginalPx = pxFromDp(
                    res.getInteger(R.integer.oplusIconDrawablePaddingDp), mMetrics);
        }
        ...
        // Oppo General: oplusGridCellPaddingDp=2 (WorkspaceParam.workspaceCellPaddingXPx).
        workspaceCellPaddingXPx = mUseOppoWorkspaceMetrics
                ? pxFromDp(res.getInteger(R.integer.oplusGridCellPaddingDp), mMetrics)
        ...
        hotseatTextSizePx = res.getDimensionPixelSize(R.dimen.hotseat_text_height_margin);
        ...
        onCreateDotRenderer(context, context.getResources().getBoolean(R.bool.config_show_notification_number));
    }

    public void onCreateDotRenderer(@NonNull Context context, boolean showNumber) {
        Path shapePath = GraphicsUtils.getShapePath(context, 100);
        NumberDotRenderer dotRenderer = new NumberDotRenderer(this.iconSizePx, shapePath, 100, showNumber, context);
        this.mDotRendererWorkSpace = dotRenderer;
        int iconSizePx1 = this.iconSizePx;
        int iconSizePx2 = this.allAppsIconSizePx;
        if (iconSizePx1 != iconSizePx2) {
            dotRenderer = new NumberDotRenderer(iconSizePx2, shapePath, 100, showNumber, context);
        }
        this.mDotRendererAllApps = dotRenderer;
    }
    ...
    private int updateAvailableDimensions(Context context) {
        ...
        // Oppo IconParam.getIconTextSizeBaseCol: 5-col phone uses iconTextSize - 3sp.
        float invIconTextSizeSp = inv.iconTextSize[mTypeIndex];
        if (mUseOppoWorkspaceMetrics && inv.numColumns == 5) {
            invIconTextSizeSp -= 3f;
        }
    ...
    public float getWorkspaceSpringLoadScale(Context context) {
        ...
        } else if (context.getResources().getBoolean(R.bool.config_show_scroll_effect)) {
            scale *= maxAvailableWidth / scaledWorkspaceWidth;
        }
        return scale;
    }
    ...
    public boolean isVerticalBarLayout() {
        ///hxy:Modify at 2024.01.13 10:21:28 by cheng_wq{{&&
        if (SystemProperties.get("persist.sys.taskbar.enable","1").equals("0")){
            return false;
        }
		///&&}}
    ...
    public int getCellContentHeight(@ContainerType int containerType) {
        switch (containerType) {
            case CellLayout.WORKSPACE:
                // Oppo DeviceProfile.getCellContentHeight(WORKSPACE) → CellLayoutParam contentHeight.
                return mUseOppoWorkspaceMetrics ? getOppoWorkspaceContentHeight() : cellHeightPx;
