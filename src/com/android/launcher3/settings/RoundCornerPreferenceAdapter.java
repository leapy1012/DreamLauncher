...
public class RoundCornerPreferenceAdapter extends PreferenceGroupAdapter {
    ...
    public void onBindViewHolder(@NonNull PreferenceViewHolder holder, int position) {
        ...
        android.util.Log.d("updateBackground", "position:" + position);
        // todo
    ...
    protected @DrawableRes int getRoundCornerDrawableRes(int position, boolean isSelected) {
        ...
        if (((CornerType & ROUND_CORNER_TOP) != 0) && ((CornerType & ROUND_CORNER_BOTTOM) == 0)) {
            // the first
            return isSelected
                    ? R.drawable.settings_round_background_top_selected
                    : R.drawable.settings_round_background_top;
        } else if (((CornerType & ROUND_CORNER_BOTTOM) != 0)
                && ((CornerType & ROUND_CORNER_TOP) == 0)) {
            // the last
            return isSelected
                    ? R.drawable.settings_round_background_bottom_selected
                    : R.drawable.settings_round_background_bottom;
        } else if (((CornerType & ROUND_CORNER_TOP) != 0)
                && ((CornerType & ROUND_CORNER_BOTTOM) != 0)) {
            // the only one preference
            return isSelected
                    ? R.drawable.settings_round_background_selected
                    : R.drawable.settings_round_background;
        } else {
            // in the center
            return isSelected
                    ? R.drawable.settings_round_background_center_selected
                    : R.drawable.settings_round_background_center;
        }
    }
        ...
        for (int i = 0; i < groupSize; i++) {
            ...
            if (group instanceof PreferenceCategory) {
                if (pref instanceof PreferenceCategory) {
                    android.util.Log.d("mappingPreferenceGroup", "PreferenceCategory");
                    ...
                    android.util.Log.d("mappingPreferenceGroup", "Preference");
                    visibleList.add(value);
                }
            } else {
                android.util.Log.d("mappingPreferenceGroup", "not PreferenceCategory");
                visibleList.add(value);
                if (pref instanceof PreferenceCategory) {
                    mappingPreferenceGroup(visibleList, (PreferenceCategory) pref);
                }
    ...
    private void updateBackground(PreferenceViewHolder holder, int position) {
        ...
        android.util.Log.d("updateBackground", "backgroundRes:" + backgroundRes);
