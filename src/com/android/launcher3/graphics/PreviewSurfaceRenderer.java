...
public class PreviewSurfaceRenderer {
    ...
    private void loadModelData() {
        ...
        if (GridSizeMigrationUtil.needsToMigrate(inflationContext, mIdp)) {
            ...
            //         mContext);
            LauncherAppState.getInstance(previewContext)
                    .getModel().getModelDbController().loadDefaultFavoritesIfNecessary();
            // LauncherAppState.getInstance(previewContext)
                ...
                public void run() {
                    ...
                    String query =
                            LauncherSettings.Favorites.SCREEN + " = " + Workspace.FIRST_SCREEN_ID
                                    ...
                                    + LauncherSettings.Favorites.CONTAINER_HOTSEAT + " or " + LauncherSettings.Favorites.CONTAINER + " != "
                                    + -1;
                    if (deviceProfile.isTwoPanels) {
                        query += " or " + LauncherSettings.Favorites.SCREEN + " = "
                                + Workspace.SECOND_SCREEN_ID;
                    }
                    Log.d("liu-db", "loadWorkspace: query = " + query);
