...
public class FloatingIconView extends FrameLayout implements
    ...
    public static void finishExitFloatingIconView(Launcher launcher) {
        if (launcher != null) {
            DragLayer dragLayer = launcher.getDragLayer();
            ViewGroup p = (ViewGroup) dragLayer.getParent();
            for (int i = p.getChildCount() - 1; i >= 0; i--) {
                View child = p.getChildAt(i);
                if (child instanceof FloatingIconView) {
                    ((FloatingIconView) child).finish(dragLayer);
                }
            }
        }
    }
