...
public class PopupDataProvider implements NotificationListener.NotificationsChangedListener {
    ...
    private Map<String, Boolean> mPackageNameBadge = new HashMap<>();
    ...
    public PopupDataProvider(Consumer<Predicate<PackageUserKey>> notificationDotsChangeListener, Context context) {
        ...
        mPackageNameBadge = gson.fromJson(json, type);
    }
    ...
    public void onNotificationPosted(PackageUserKey postedPackageUserKey,
        ...
        Log.d(TAG, "onNotificationPosted " + postedPackageUserKey);
        DotInfo dotInfo = mPackageUserToDotInfos.get(postedPackageUserKey);
        if (dotInfo == null) {
            Log.d(TAG, "onNotificationPosted: no dot info for " + postedPackageUserKey);
            dotInfo = new DotInfo();
            mPackageUserToDotInfos.put(postedPackageUserKey, dotInfo);
        }
        if (dotInfo.addOrUpdateNotificationKey(notificationKey)) {
            Log.d(TAG, "onNotificationPosted: updated dot info for " + postedPackageUserKey);
    ...
    public @Nullable boolean getBadgeInfoForItem(ItemInfo info) {
        boolean bRet = false;
        if (info == null || !ShortcutUtil.supportsShortcuts(info)) {
            return bRet;
        }
        if (info.getTargetComponent() != null && mPackageNameBadge != null) {
            if(mPackageNameBadge.get(info.getTargetComponent().getPackageName()) != null) {
                bRet = mPackageNameBadge.get(info.getTargetComponent().getPackageName());
            }
        }
        return bRet;
    }
    ...
    public void onShortcutCreated(String pkgName, Launcher launcher) {
        if (mPackageNameBadge == null) {
            mPackageNameBadge = new HashMap<>();
        }
        mPackageNameBadge.put(pkgName, true);
        Gson gson = new Gson();
        String json = gson.toJson(mPackageNameBadge);
        launcher.getSharedPrefs().edit().putString(KEY_BADGE_SHOW, json).apply();
    }

    public void onShortcutBadgeRemoved(String pkgName, Launcher launcher) {
        if (mPackageNameBadge == null) {
            return;
        }
        mPackageNameBadge.remove(pkgName);
        Gson gson = new Gson();
        String json = gson.toJson(mPackageNameBadge);
        launcher.getSharedPrefs().edit().putString(KEY_BADGE_SHOW, json).apply();
    }
    ...
    public boolean checkBadgeShow(String pkgName) {
        ...
        return mPackageNameBadge.get(pkgName) != null;
    }
}
