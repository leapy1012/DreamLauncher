...
///&&}}
///Hxy:add for memory expansion at 20240102{{&&
...
public class OverviewActionsView<T extends OverlayUICallbacks> extends FrameLayout
	...
	///Hxy:add for modify clearallbutton location at 20240102{{&&
	public View getClearAllView(){
		View v = findViewById(R.id.action_clearall);
		return v;
	}
	public void updateMeeryinfoDisplay(){
        ...
        if(android.os.SystemProperties.get("persist.sys.hxycustom.memory_expansion_size","-1").trim().equals("-1")){
            if(memeryinfo != null ){
                memeryinfo.setText(""+ df1.format(availMemory) + "GB" +" "+getResources().getString(R.string.hxy_launcher_available)+" | " + totalMemory);
            }
        }else{
            int hxy_memory_expansion_enable = android.provider.Settings.System.getInt(mContext.getContentResolver(),"hxy_memory_expansion_enable",0);
            Float availSwapAndMemory = 0f;
            if(hxy_memory_expansion_enable == 1){
                ...
                if (!TextUtils.isEmpty(expand)) {
                    availSwapAndMemory = availMemory + Long.parseLong(expand);
                } else {
                    availSwapAndMemory = availMemory;
                }
                if(memeryinfo != null ){
                    memeryinfo.setText(""+ df1.format(availSwapAndMemory) + "GB "+getResources().getString(R.string.hxy_launcher_available)+" | " 
                    + totalMemory.replaceAll("GB", " + ") + android.os.SystemProperties.get("persist.sys.hxycustom.memory_expansion_size","1") + "GB");
                }
            }else{
                if(memeryinfo != null ){
                    memeryinfo.setText(""+ df1.format(availMemory) + "GB" +" "+getResources().getString(R.string.hxy_launcher_available)+" | " + totalMemory);
    ...
    private int getBottomMargin() {
        ...
        //android.util.Log.i("cwq"," config_clearall_center:"+mContext.getResources().getBoolean(R.bool.config_clearall_center)+" mDp.isLandscape:"+mDp.isLandscape);
        if(mContext.getResources().getBoolean(R.bool.config_clearall_center))
        {
            tempHeight = mDp.isLandscape ? 10 : 0;
        }
        // Align to bottom of task Rect.
        return mDp.heightPx - mTaskSize.bottom - mDp.overviewActionsTopMarginPx
                - mDp.overviewActionsHeight + tempHeight;
    }
	...
	///Hxy:add for memory expansion at 20240102{{&&
    public static synchronized String runShell(String cmd) throws IOException {
        String line = null;
