...
public class IconProvider {
    ...
    public Drawable getIcon(LauncherActivityInfo info, int iconDpi) {
        return getIconWithOverrides(info.getComponentName(), iconDpi,
    ...
    public Drawable getIcon(ActivityInfo info, int iconDpi) {
        return getIconWithOverrides(new ComponentName(info.packageName, info.name), iconDpi,
                () -> loadActivityInfoIcon(info, iconDpi));
    }

    public Drawable getIcon(ComponentName componentName) {
        return getIconWithOverrides(componentName, mContext.getResources().getConfiguration().densityDpi);
    }
            ...
            if ("com.android.dialer/com.android.contacts.ui.main.ContactMainActivity".equals(
                ...
                componentName.flattenToShortString())) {
                    ...
                    if (td == null) {
                        try {
                            PackageManager pm = mContext.getPackageManager();
                            int resId = pm.getResourcesForApplication(packageName).getIdentifier("camera_scan", "drawable", packageName);
                            if (resId > 0) {
                                icon = pm.getResourcesForApplication(packageName).getDrawable(resId, null);
                            }
                        } catch (NameNotFoundException | Resources.NotFoundException exc) { }
                    }
            } else if ("com.android.toolbox/.flashlight.FlashlightActivity".equals(
                componentName.flattenToShortString())) {
                    td = getThemeDataForPackage("com.android.toolbox.flashlight");
                    if (td == null) {
                        try {
                            PackageManager pm = mContext.getPackageManager();
                            int resId = pm.getResourcesForApplication(packageName).getIdentifier("icon_flashlight", "drawable", packageName);
                            if (resId > 0) {
                                icon = pm.getResourcesForApplication(packageName).getDrawable(resId, null);
                            }
                        } catch (NameNotFoundException | Resources.NotFoundException exc) { }
                    }
            } else if ("com.android.launcher3/.big.memoryclean.MemoryCleanActivity".equals(
                componentName.flattenToShortString())) {
                    td = getThemeDataForPackage("com.android.launcher3.clean");
                    if (td == null) {
                        try {
                            PackageManager pm = mContext.getPackageManager();
                            int resId = pm.getResourcesForApplication(packageName).getIdentifier("hxy_clear_bg", "mipmap", packageName);
                            if (resId > 0) {
                                icon = pm.getResourcesForApplication(packageName).getDrawable(resId, null);
                            }
                        } catch (NameNotFoundException | Resources.NotFoundException exc) { }
                    }
            } else if ("com.android.launcher3/.settings.WallpaperChangeActivity".equals(
                componentName.flattenToShortString())) {
                    td = getThemeDataForPackage("com.android.launcher3.wallpaper");
                    if (td == null) {
                        try {
                            PackageManager pm = mContext.getPackageManager();
                            int resId = pm.getResourcesForApplication(packageName).getIdentifier("choosewallpapaer_bg", "mipmap", packageName);
                            if (resId > 0) {
                                icon = pm.getResourcesForApplication(packageName).getDrawable(resId, null);
                            }
                        } catch (NameNotFoundException | Resources.NotFoundException exc) { }
            ...
            if ("com.android.dialer/com.android.contacts.ui.main.ContactMainActivity".equals(
                ...
                componentName.flattenToShortString())) {
                    try {
                        PackageManager pm = mContext.getPackageManager();
                        int resId = pm.getResourcesForApplication(packageName).getIdentifier("camera_scan", "drawable", packageName);
                        android.util.Log.d("IconProvider", "getIconWithOverrides: " + resId);
                        if (resId > 0) {
                            icon = pm.getResourcesForApplication(packageName).getDrawable(resId, null);
                        }
                    } catch (NameNotFoundException | Resources.NotFoundException exc) { }
        ...
        try {
            ...
            if (id != ID_NULL) {
                ...
                if (themedName.contains("hills")) {
                    ta = mContext.getResources().obtainTypedArray(R.array.theme_hills_calendar_img_array);
                } else if (themedName.contains("tunnel")) {
                    ta = mContext.getResources().obtainTypedArray(R.array.theme_tunnel_img_array);
                } else if (themedName.contains("forest")) {
                    ta = mContext.getResources().obtainTypedArray(R.array.theme_forest_calendar_img_array);
                } else if (themedName.contains("winter")) {
                    ta = mContext.getResources().obtainTypedArray(R.array.theme_winter_img_array);
                } else if (themedName.contains("golden")) {
                    ta = mContext.getResources().obtainTypedArray(R.array.theme_golden_calendar_img_array);
                } else if (themedName.contains("night")) {
                    ta = mContext.getResources().obtainTypedArray(R.array.theme_night_calendar_img_array);
                } else if (themedName.contains("minimalist")) {
                    ta = mContext.getResources().obtainTypedArray(R.array.theme_default_calendar_img_array);
                    ...
                    ta = mContext.getResources().obtainTypedArray(R.array.calendar_img_array);
                }
                int resId = ta.getResourceId(getDay(), ID_NULL);
                ta.recycle();
                return resId == ID_NULL ? null : mContext.getResources().getDrawable(resId, null);
            }
    ...
    public static class ThemeData {
        ...
        //hxy-feature: desktop theme 202312
        Drawable loadIcon(){
            return mResources.getDrawable(mResID);
        }
        //hxy-feature: desktop theme 202312
