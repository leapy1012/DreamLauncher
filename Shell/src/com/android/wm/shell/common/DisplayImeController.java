...
public class DisplayImeController implements DisplayController.OnDisplaysChangedListener {
    ...
    //"true" means rotate screen when the screen is split-screen and the IME is displayed.
    ...
    public void onDisplayConfigurationChanged(int displayId, Configuration newConfig) {
        ...
        if (mSplitScreenRotating) {
            pd.mRotatingMessages.add(new Pair<>(PerDisplay.MSG_DISPLAY_CONFIGURATION_CHANGED, ()->{
                onDisplayConfigurationChanged(displayId, newConfig);
            }));
            return;
        }
    ...
    public class PerDisplay implements DisplayInsetsController.OnInsetsChangedListener {
        ...
        static final int MSG_SHOW_INSETS = 0;
        static final int MSG_HIDE_INSETS = 1;
        static final int MSG_INSETS_CHANGED = 2;
        static final int MSG_INSETS_CONTROL_CHANGED = 3;
        static final int MSG_DISPLAY_CONFIGURATION_CHANGED = 4;
        ArrayList<Pair<Integer, Runnable>> mRotatingMessages = new ArrayList<>();
        //Can not seek ime animation when set to true.
        boolean mReStartImeAnimating = false;
        //"true" means Split-screen is middle position. Thus it do not need IME animation.
        boolean mTransitionMidPosition = true;
        ...
        public void insetsChanged(InsetsState insetsState) {
            ...
            if (mSplitScreenRotating){
                mRotatingMessages.add(new Pair<>(MSG_INSETS_CHANGED, () -> {
                    insetsChanged(insetsState);
                }));
                return;
            }
            boolean mOldImeShowing = mImeShowing;
            ...
            if (mReStartImeAnimating) {
                if (mImeShowing != mOldImeShowing) {
        ...
        public void insetsControlChanged(InsetsState insetsState,
            ...
            if (mSplitScreenRotating){
                mRotatingMessages.add(new Pair<>(MSG_INSETS_CONTROL_CHANGED, () -> {
                    insetsControlChanged(insetsState, activeControls);
                }));
                return;
            }
            insetsChanged(insetsState);
        ...
        public void showInsets(@InsetsType int types, boolean fromIme,
            ...
            if (DEBUG) Slog.d(TAG, "Got showInsets for ime, mSplitScreenRotating=" +
                    mSplitScreenRotating);
            if (mSplitScreenRotating){
                mRotatingMessages.add(new Pair<>(MSG_SHOW_INSETS,()->{
                    showInsets(types, fromIme, statsToken);
                }));
                return;
            }
        ...
        public void hideInsets(@InsetsType int types, boolean fromIme,
            ...
            if (DEBUG) Slog.d(TAG, "Got hideInsets for ime, mSplitScreenRotating=" +
                    mSplitScreenRotating);
            if (mSplitScreenRotating){
                mRotatingMessages.add(new Pair<>(MSG_HIDE_INSETS,()->{
                    hideInsets(types, fromIme, statsToken);
                }));
                return;
            }
        ...
        private void startAnimation(final boolean show, final boolean forceRestart,
            ...
            final Rect oldFrame = new Rect(mImeFrame);
            if (isFloating) {
            ...
            if (oldFrame.equals(mImeFrame) && mImeShowing == show && !forceRestart &&
                    !mReStartImeAnimating){
                if (DEBUG) {
                    Slog.d(TAG, "All same and do not force restart. Do not need start animation");
                }
                return;
            }
            if (DEBUG) {
                Slog.d(TAG, "Run starImetAnim  show:" + show + "  was:"
        ...
        public void reStartImeAnimation() {
            mReStartImeAnimating = true;
            boolean needHideIme = false;
            for (int i = mRotatingMessages.size() -1; i>=0; i--){
                Pair<Integer, Runnable> pair = mRotatingMessages.get(i);
                if (pair.first == MSG_HIDE_INSETS || pair.first == MSG_SHOW_INSETS){
                    needHideIme = pair.first == MSG_HIDE_INSETS;
                    mRotatingMessages.remove(pair);
                    break;
                }
            }
            if (needHideIme){
                startAnimation(false, !mTransitionMidPosition,null);
            } else {
                for (int i = mRotatingMessages.size() -1; i>=0; i--) {
                    Pair<Integer, Runnable> pair = mRotatingMessages.get(i);
                    if (pair.first == MSG_INSETS_CHANGED || pair.first == MSG_SHOW_INSETS) {
                        pair.second.run();
                        break;
                    }
                }
            }
            mRotatingMessages.clear();
            mReStartImeAnimating = false;
        }

        public void setTransitionMidPosition(boolean transitionMidPosition) {
            mTransitionMidPosition = transitionMidPosition;
            if (mImeShowing && mImeSourceControl != null && mImeSourceControl.getLeash() != null &&
            mImeSourceControl.getLeash().isValid() && mTransitionMidPosition) {
                mImeShowing = false;
                SurfaceControl.Transaction t = mTransactionPool.acquire();
                t.setPosition(mImeSourceControl.getLeash(), 0,
                        -mImeSourceControl.getSurfacePosition().y);
                t.setAlpha(mImeSourceControl.getLeash(), 1.f);
                t.hide(mImeSourceControl.getLeash());
                t.apply();
            }
    ...
    public void reStartImeAnimation(int displayId) {
        ...
        pd.reStartImeAnimation();
    }

    public void setRotating(boolean rotating, int displayId) {
        PerDisplay pd = mImePerDisplay.get(displayId);
        if (pd == null) {
            return;
        }
        mSplitScreenRotating = rotating;
        if (mSplitScreenRotating) {
            pd.startAnimation(false, true, null);
        }
    }

    public void setTransitionMidPosition(boolean transitionMidPosition, int displayId) {
        PerDisplay pd = mImePerDisplay.get(displayId);
        if (pd == null) {
            return;
        }
        pd.setTransitionMidPosition(transitionMidPosition);
    }
