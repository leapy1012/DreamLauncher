...
public class StageCoordinator implements SplitLayout.SplitLayoutHandler,
    ...
    private Transitions.TransitionObserver mTransitionObserver;
    ...
    protected void onSplitScreenEnter() {
        mDisplayImeController.setRotating(false, mContext.getDisplayId());
    }
    ...
    public boolean startAnimation(@NonNull IBinder transition,
        ...
        if (!mSplitTransitions.isPendingTransition(transition)) {
            ...
            int topTaskId = getTaskId(SPLIT_POSITION_TOP_OR_LEFT);
            boolean midPosition = true;
            for (int iC = 0; iC < info.getChanges().size(); ++iC) {
                ...
                if (stage.mRootTaskInfo.taskId == topTaskId){
                    midPosition = change.getEndAbsBounds().top > 0;
                }
                if (isOpeningType(change.getMode())) {
            ...
            if (mDisplayImeController.mSplitScreenRotating) {
                mDisplayImeController.setTransitionMidPosition(midPosition, mDisplayId);
            }
            if (mTransitionObserver == null && mDisplayImeController.mSplitScreenRotating) {
                mTransitionObserver = new Transitions.TransitionObserver() {
                    @Override
                    ...
                    public void onTransitionFinished(IBinder transition, boolean aborted) {
                        mTransitions.unregisterObserver(mTransitionObserver);
                        mTransitionObserver = null;
                        if (mDisplayImeController.mSplitScreenRotating) {
                            mDisplayImeController.mSplitScreenRotating = false;
                            mDisplayImeController.reStartImeAnimation(mDisplayId);
                            onLayoutSizeChanged(mSplitLayout);
                        }
                    }
                };
                mTransitions.registerObserver(mTransitionObserver);
            }
